          ...
          'parts': [
            {
              'text': _questionCount < 4
                  ? '너는 지금 내 식사 메뉴를 추천해주는 ai야 질문들을 통해서 나의 답변을 기반으로 나에게 가장 알맞은 메뉴를 추천해줘. 질문은 다섯가지만 하고, 아래는 너가 지켜야 할 주의사항들이야1. 질문은 한번에 하나씩 한다.2. 질문에 대한 내 답변으로 추천할 메뉴의 경우의 수를 줄여나간다.3. 질문들에는 항상 1번부터 5번까지 보기가 있어야해 4. 외식, 배달, 집에서 해먹는 것을 가장 큰 카테고리로 두고 선택하라 한다. 5. 마지막 질문 후 너가 메뉴를 추천하기 전까지 질문과 관계없는 답변에는 다시 답변해달라고 부탁한다. 6. 중복된 내용의 질문은 하지 말아. '
                  '${_previousQuestion}\n'
                  '${_controller.text}'
                  : '사용자가 한 답변들을 바탕으로 적절한 메뉴를 추천해줘 아래 지시사항을 지키도록해 1.안내멘트는 추천해드릴 메뉴는 ~입니다. 라는 형식을 따르도록해 2.정확한 메뉴 이름을 알려줘야 한다.(ex 엽떡, 쌀국수, 까르보나라 등) 3. 가능하면 메뉴와 함께 해당 한국에 있는 브랜드도 추천해주면 더 좋아 4. 전 질문과 비슷한 내용은 질문하지마 5.이모티콘이나 기호를 사용하지마: ${_previousQuestion}\n${_controller.text}' // 5번째는 메뉴를 추천
            }
          ]
        }
      ]
